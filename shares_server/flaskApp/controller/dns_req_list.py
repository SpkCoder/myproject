...
class model(object):
    ...
    def find_data(self):
        ...
        if time_interval == 3600:
            sql = 'select date_format(log_time,"%Y-%m-%d %H:00:00") as time, type_name, count(*) as num from '+union_sql+' group by time,type_name order by time asc'
        if time_interval == 300:
            sql = 'select create_time as time, type_name, count(*) as num from '+union_sql+' group by create_time,type_name order by time asc'
        if time_interval == 60:
            sql = 'select date_format(log_time,"%Y-%m-%d %H:%i:00") as time, type_name, count(*) as num from '+union_sql+' group by time,type_name order by time asc'
