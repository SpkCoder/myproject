...
router.get('/', function(req, res, next) {
	...
	function findData () {
		...
		function findDataLookup_productClass () {
		    return new Promise(function(resolve, reject){   
		        var fromJson={"dbName":"productClass","pk":"productClass_id","fk":"id","as":"lookup"};
		...
		function findDataLookup_braceClass () {
		    return new Promise(function(resolve, reject){   
		        var fromJson={"dbName":"modelClass","pk":"modelClass_id","fk":"id","as":"lookup"};
				db.findDataLookup(dbName, fromJson, whereJson, fieldJson, args, function(err, result, count){
				        if(err){
				            returnErr('findDataError:'+ err);
				            reject("Error");
		...
		Promise
		.all([findDataLookup_productClass(), findDataLookup_braceClass()])
		.then(function(result){
		    //console.log(result);
		    var jsonObj = result[0];
		    var jsonObj2 = result[1];
		    jsonObj.rows.forEach(function(item, index) {
		    	item.productClass_name = item.lookup[0].class_name;
		    	item.modelClass_name = jsonObj2.rows[index].lookup[0].class_name;
		    	delete item.lookup;
		    });
		    //console.log(jsonObj);
		    res.send(jsonObj);
...
router.post('/', function(req, res, next) {
	...
	function insertData () {
		...
		if(dataArr.length > 1){
			res.send("The length of dataArr must be 1");
			return;
		}
		
		for(let i=0; i<dataArr.length; i++){
			if(!dataArr[i].id || !dataArr[i].productClass_id || !dataArr[i].modelClass_id){
				res.send("id and productClass_id and modelClass_id can not be empty");
				return;
			}
		...
		    return new Promise(function(resolve, reject){   
		        var whereJson = {"id": dataArr[0].id};
				var fieldJson = {"id": 1, "name":1, "productClass_id":1};
				var args = "";
				db.findData(dbName, whereJson, fieldJson, args, function(err, result, count){
				       ...
				       console.log(result);
				       if(result.length > 0){
				       	   if(dataArr[0].productClass_id != result[0].productClass_id){
					       	 res.send("The ProductClass must be unique");
					       	 reject("false");
					       }else{
					       	 dataArr[0].name = result[0].name;
					       	 resolve("true");
					       }
				       }else{
				       	 resolve("true");
		...
		function testModelClass (data) {
		    return new Promise(function(resolve, reject){   
		        var whereJson = {"id": dataArr[0].id, "modelClass_id": dataArr[0].modelClass_id};
				var fieldJson = {"id": 1, "modelClass_id":1};
				var args = "";
				db.findData(dbName, whereJson, fieldJson, args, function(err, result, count){
				        if(err){
				            returnErr('findDataError:'+ err);
				       ...
				       //console.log(result);
				       if(result.length > 0){
				       	 res.send("ModelClass has already existed");
				       	 reject("false");
				       }else{
				       	 resolve("true");
				       }
