 ...
 * @param y         y坐标
 */
function cropCurrentImg(srcImg,width, height, x, y,callback) {
 ...
 * @param y         y坐标
 */
function cropImg(srcImg,destImg,width, height, x, y,callback) {
 ...
 */
function resizeCurrentImg(srcImg, size,callback) {
 ...
 * @param size      缩放后的图片大小(长宽均为size)
 */
function resizeImgWithArgs(srcImg, destImg, size,callback) {
 ...
 * @param y         水印y坐标
 */
function addWaterMark(srcImg,watermarkImg,destImg,x,y,callback){
    gm(srcImg).composite(watermarkImg).geometry('+'+x+'+'+y).write(destImg, function(err) {
	    callback(err);
	});
}
