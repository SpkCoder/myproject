...
exports.getPower = function (UserName, dbName, action, callback) {
    ...
    var limit = 0;    //数目限制
    var sort = {};    //排序方式
    var fields = {"UserName":1, "roleClass_id":1};  //过滤字段
    ...
    _connectDB(function (db) {
    	db.collection("userList").find(whereJson, fields).skip(skipNum).limit(limit).sort(sort).toArray(function(err, result) {
	        ...
	        var role_id = result[0].roleClass_id;
	        var whereJson2 = {"role_id": role_id, "dbName": dbName, "function": action};
	        db.collection("rolePermissionList").count(whereJson2, function (err, count) {
	        	count = [1];
	    		callback(err,count);
		        db.close(); //关闭数据库
	        });
...
//修改数据多行
exports.updateDataMany = function (collectionName, whereJson, updateJson, callback) {
    _connectDB(function (db) {
        db.collection(collectionName).update(whereJson, updateJson, {"multi" : true}, function (err, result) {
                callback(err, result);
                db.close();//关闭数据库
        });
    })
