...
router.get('/', function(req, res, next) {
	...
	function findData () {
		...
		var fromJson={"dbName":"optionsValueClass","pk":"value_id","fk":"id","as":"lookup"};
		db.findDataLookup(dbName, fromJson, whereJson, fieldJson, args, function(err, result, count){
		       ...
		       jsonObj.rows.forEach(function(item, index) {
			    	item.name = item.lookup[0].name;
			    	item.value = item.lookup[0].value;
			    	delete item.lookup;
			   });
		       res.send(jsonObj);
		});
...
router.post('/', function(req, res, next) {
	...
	function insertData () {
		...
		for(let i=0; i<dataArr.length; i++){
            ...
            value_idArr.push(dataArr[i].value_id);
		}
		var whereJson = {"productId_modelClassId_optionId":dataArr[0].productId_modelClassId_optionId, "value_id":{"$in":value_idArr}};
		var fieldJson = {"productId_modelClassId_optionId": 1, "value_id": 1};
		var args = "";
		db.findData(dbName, whereJson, fieldJson, args, function(err, result, count){
		        ...
		        //console.log(result);
		        if(result.length > 0){
		       	 res.send("Value has already existed");
				 return;
		        }
	...
	function updateData () {
		...
		if(updateData.isDefault == "true"){
			...
			db.updateDataMany(dbName, {"productId_modelClassId_optionId": whereJson.productId_modelClassId_optionId}, {$set: {"isDefault": "false"}}, function(err2,result2){
			        if(err2){
