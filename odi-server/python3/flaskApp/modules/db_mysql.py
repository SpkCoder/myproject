...
class model():
    ...
    def _db_connect(self):
        try:
            ...
            logging.error('连接数据库mysql: ' + str(e))
            exit()
    ...
    def find_data(self, table_name, params={}, sql='', sql_count=''):
        ...
        try:
            ...
            #获取count
            if sql_count:
                sql_count_start_time = time.time()
                cursor.execute(sql_count)
                count = cursor.fetchone()['number']
                sql_count_end_time = time.time()
                sql_count_time = sql_count_end_time - sql_count_start_time
            else:
                count = 0
                sql_count_time = 0
            #获取rows
            ...
            logging.debug('sql_count_time: ' + str(sql_count_time))
            logging.debug('sql_time: ' + str(sql_time))
            self._db.close()
            del cursor
    ...
    def insert_data(self, table_name, list_data):
        ...
        try:
            ...
            if len(tuple_title) == 1:
                str_title = re.sub(r'\,', '', str_title)
            sql += str_title
            sql += ' values '
            int_num = 1
            ...
            self._db.rollback()
            self._db.close()
            logging.error('insert_data: ' + str(e))
            exit()    
    ...
    def update_data(self, table_name, whereJson, updateJson):
        ...
        try:
            ...
            self._db.rollback()
            self._db.close()
            logging.error('update_data: ' + str(e))
            exit()    
    ...
    def execute(self, sql):
        ...
        try:
            ...
            self._db.rollback()
            self._db.close()
            logging.error('execute: ' + str(e))
            exit()

    #初始化分表
    def split_table(self, table_name):
        try:
            sql = 'select * from '+table_name+' order by create_time desc limit 0,1'
            result = self.find_data('', {}, sql)
            table_name2 = result['rows'][0]['table_name']
            sql = 'select count(*) as number from '+table_name2
            result = self.find_data('', {}, sql)
            number = result['rows'][0]['number']
            if number >= SPLIT_NUMBER:
                list_table_name2 = table_name2.split("_")
                new_table_name = list_table_name2[0] +'_'+ list_table_name2[1] +'_'+ list_table_name2[2] +'_'+ str(int(list_table_name2[3])+1)
                sql = 'create table %s like %s' % (new_table_name, table_name2)
                result = self.execute(sql)
                sql = 'insert into %s (table_name, create_time) values ("%s", "%s")' % (table_name, new_table_name, time.strftime('%Y-%m-%d %H:%M:%S',time.localtime()))
                result = self.execute(sql)
                logging.info('create table: ' + new_table_name)
                return new_table_name
            else:
                return table_name2

        except Exception as e:
            logging.error('split_table: ' + str(e))
            exit()

    #get_union_sql
    def get_union_sql(self, table_name, str_gte, str_lt, whereStr):
        try:
            sql = 'select * from '+table_name+' order by create_time asc'
            result = self.find_data('', {}, sql)
            union_sql = ''
            prev_row_index = -2
            for index, item in enumerate(result['rows']):
                int_create_time = time.mktime(time.strptime(item['create_time'], "%Y-%m-%d %H:%M:%S"))
                int_gte = time.mktime(time.strptime(str_gte, "%Y-%m-%d %H:%M:%S"))
                int_lt = time.mktime(time.strptime(str_lt, "%Y-%m-%d %H:%M:%S"))
                if int_create_time >= int_gte and int_create_time < int_lt:
                    if prev_row_index == -2:
                        prev_row_index = index-1
                    if whereStr:
                        union_sql += 'select * from %s where %s union all ' % (item['table_name'], whereStr)
                    else:
                        union_sql += 'select * from %s union all ' % (item['table_name'])
            if prev_row_index >= 0:
                # 拼接上一个table
                prev_row = result['rows'][prev_row_index]
                if whereStr:
                    union_sql = 'select * from %s where %s union all ' % (prev_row['table_name'], whereStr) + union_sql
                else:
                    union_sql = 'select * from %s union all ' % (prev_row['table_name']) + union_sql
            else:
                prev_row = result['rows'][len(result['rows'])-1]
                if whereStr:
                    union_sql = 'select * from %s where %s union all ' % (prev_row['table_name'], whereStr)
                else:
                    union_sql = 'select * from %s union all ' % (prev_row['table_name'])
            union_sql = '(' + union_sql[0:len(union_sql)-10] + ') utable'
            return union_sql
        except Exception as e:
            logging.error('get_union_sql: ' + str(e))
            exit()
