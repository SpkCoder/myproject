...
function _searchCommon($http,$scope,checkType){
    ...
    var ComSearchHref=http+"/handler/project/project/list.ashx?"+idStr;
   ...
   $scope._getComSearch();
    ...
    $scope.getComSearchCondition = function(){
        ...
        if($scope.search_dept_id){
            seaCondition += "dept_id="+$scope.search_dept_id;
        }
        
        if(seaCondition){
            if($scope.search_apply_user_id){
              seaCondition += " and apply_user_id="+$scope.search_apply_user_id;
            }
        }else{
            if($scope.search_apply_user_id){
              seaCondition += "apply_user_id="+$scope.search_apply_user_id;
            }
        }
        
        if(seaCondition){
            if($("#create_time_start").val()){
              seaCondition += " and create_time>='" +$("#create_time_start").val()+ "'";
            }
        }else{
            if($("#create_time_start").val()){
              seaCondition += "create_time>='" +$("#create_time_start").val()+ "'";
            }
        }
        ...
        if(seaCondition){
            if($("#create_time_end").val()){
              seaCondition += " and create_time<='" +$("#create_time_end").val()+ "'";
            }
        }else{
            if($("#create_time_end").val()){
              seaCondition += " and create_time<='" +$("#create_time_end").val()+ "'";
   ...
   //查询
    ...
    $scope.getComSearch_cancel = function(){
        ...
        $scope.search_dept_id = '';
        $scope.search_apply_user_id = '';
        $("#search_dept_name").val("");
        $('#search_apply_user_name').val("");
        ...
        $scope.ComSearchCondition = "status=1";
        $scope.ComCheckedId = "";
        $scope.ComCheckedData = "";
        $scope.ComCheckedArr = [];
        $scope.ComCheckedDataArr = [];
