		...
		<div class="list-page-box clearfix">
		  	  <div class="container clearfix">
                  ...
                  v-loading="listLoading"
									...
									<template v-for='(item, index) in field_en'>
											<el-table-column v-if='item == "address_rate"' :key="item.id" show-overflow-tooltip sortable="custom" :prop="item" :label="field_ch[index]" :width="field_width[index] | field_width_filter">
                        <template slot-scope="scope">
                          {{ scope.row.address_rate | address_rate_filter}}
                        </template>
                      </el-table-column>
                  ...
                  <el-table-column fixed="right" label="操作" width="130"> 
                      <template slot-scope="scope"> 
                      <router-link :to="'/page/address_rate_list_detail/' + scope.row.client_ip">
      ...
      _this.$axiosHttp.get(_this.url, {"params":reqData}).then(function (res) {
        ...
        if(res.code != 200){
          ...
          return false;
				...
				_this.list = res.rows;
				_this.field_ch = ["网段", "地址利用率", "VLAN ID", "VLAN NAME", "VRF NAME", "时间"];
				_this.field_en = ["client_ip", "address_rate", "vlan_id", "vlan_name", "vrf_name", "log_time"];
				_this.data_type = ["text", "text", "text", "text", "text", "text"];
				_this.field_width = [120, 120, 120, 120, 120, 150];
