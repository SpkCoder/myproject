...
function _searchCommon($http,$scope,checkType){
    ...
    var ComSearchHref=http+"/handler/grade/grade_class_template/list.ashx?"+idStr;
   ...
   $scope._getComSearch();
    ...
    //加载资产设备分类
     var getClassHref = http+"/handler/objects/object_class/list.ashx?"+idStr+"&sortField=&searchCondition=status=1&pageNumber=1&pageSize="+1000000+"&totalRecord=0&action=GetPages&timeStamp=" + Date.parse(new Date());
      $http({
          method: 'GET', 
          url: getClassHref,
          dataType:"json",
    ...
    $scope.btn_getComSearch = function(){
        ...
        if(seaCondition){
            if($scope.search_items){
              seaCondition += " and items like'%"+$scope.search_items+"%'";
            }
        }else{
            if($scope.search_items){
              seaCondition += "items like'%"+$scope.search_items+"%'";
            }
        }
