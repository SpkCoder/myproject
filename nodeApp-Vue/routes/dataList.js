...
router.get('/', function(req, res, next) {
    ...
    db.getPower(username, dbName, action2, function(err, power){
	        if(err){
...
router.post('/', function(req, res, next) {
        ...
        try{
		  ...
		  console.log("复制文件"+this_dbName+".html" +"成功");
			try{
			    ...
			    console.log("复制文件"+this_dbName+".js" +"成功");
			    ...
			    fs.readFile(file_routerUrl, {encoding:'utf-8'}, function(readFile_err,readFile_data){
				    if(readFile_err){
				    	returnErr('fs.readFile:'+ readFile_err);
				        return;
				    };
			    ...
			    //给settings.js添加routerUrl
			    var file_settings = path.join(path.dirname(__dirname),"/my_modules/"+"settings.js");
			    fs.readFile(file_settings, {encoding:'utf-8'}, function(readFile_err,readFile_data){
				    ...
				    fs.writeFile(file_settings, settingsStr, {encoding:'utf-8'}, function(writeFile_err){
					    ...
					    console.log("给settings.js添加" + JSON.stringify(routerUrl) + "成功");
					});
				});
			}catch(e){
				returnErr('fs.createReadStream:'+ e);
				return;
			}
		}catch(e){
			returnErr('fs.createReadStream:'+ e);
    ...
    function removeFileFn (this_dbName) {
		...
		fs.unlink(file_dest_html,function (remove_err) {
		    if(remove_err){
	           returnErr('fs.unlink:'+ remove_err);
	           return;
	        }
		    console.log("删除文件"+this_dbName+".html" +"成功");
		    fs.unlink(file_dest_js,function (remove_err2) {
			    if(remove_err2){
		           returnErr('fs.unlink:'+ remove_err2);
		           return;
		        }
			    console.log("删除文件"+this_dbName+".js" +"成功");
			    ...
			    fs.readFile(file_settings, {encoding:'utf-8'}, function(readFile_err,readFile_data){
				    ...
				    fs.writeFile(file_settings, settingsStr, {encoding:'utf-8'}, function(writeFile_err){
					    ...
					    console.log("给settings.js删除" + JSON.stringify(routerUrl) + "成功");
					});
				});
			});
		});
