...
#/python/dns_err_list req_data { "action":"findData", "whereJson":{"time": "2019-07-13"}, "tocken": "eyJhbGciOiJIUzIbmFtZSmei4" }  //查询数据
    ...
    def find_data(self):
        ...
        if result:
            ...
            for item in result['rows']:
                result['dict_time'][item['time']] = item['num']
            # print(result['dict_time'])
            for i in range(int_num):
                str_time = datetime.fromtimestamp(int_gte + time_interval*i).strftime('%Y-%m-%d %H:%M:%S')
                # print(str_time)
                if str_time in result['dict_time']:
                    num = result['dict_time'][str_time]
                else:
                    num = 0
