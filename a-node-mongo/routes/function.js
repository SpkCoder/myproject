...
router.get('/', async function(req, res, next) {
        ...
        async function findData() {
            ...
            try {
                var count = await FunctionModel.find(whereJson).count();
                var result = await FunctionModel.find(whereJson).skip(skipNum).limit(prePageNum).sort(sortJson).select(fieldJson);
...
router.post('/', async function(req, res, next) {
        ...
        async function insertData () {
            ...
            if(!dataArr[0].permission || !dataArr[0].tableName || !dataArr[0].action){
                sendLog("permission must not be null");
                return;
            }

            try {
                var result = await FunctionModel.findOne({permission: dataArr[0].permission, tableName: dataArr[0].tableName, action: dataArr[0].action});
                if (result) {
                    sendLog("function already existed");
                    return;
                }
                var result = await new FunctionModel({
                    permission: dataArr[0].permission,
                    tableName: dataArr[0].tableName,
                    action: dataArr[0].action,
                }).save();
        ...
        async function delData () {
            ...
            try {
                var result = await FunctionModel.remove(whereJson);
                sendLog("Success");
            } catch (err) {
