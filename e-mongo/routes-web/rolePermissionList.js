...
router.get('/', function(req, res, next) {
	...
	function findData () {
		...
		var fromJson={"dbName":"roleClass","pk":"role_id","fk":"id","as":"lookup"};
...
router.post('/', function(req, res, next) {
	...
	function insertData () {
		...
		if(dataArr.length > 1){
			res.send("Do not support batch insert");
            return;
		};
		var functionArr = [];
		for(let i=0; i<dataArr.length; i++){
            ...
            functionArr.push(dataArr[i].function);
		}
		var whereJson = {"role_id":dataArr[0].role_id, "dbName":dataArr[0].dbName, "function":{"$in":functionArr}};
		var fieldJson = {"role_id": 1, "dbName": 1, "function": 1};
		var args = "";
		db.findData(dbName, whereJson, fieldJson, args, function(err, result, count){
		       ...
		       //console.log(result);
		       if(result.length > 0){
		       	 res.send("The role permissions have already existed");
				 return;
		       }
